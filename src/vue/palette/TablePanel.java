...
public class TablePanel extends JPanel {
	...
	private void initTable() {
		...
		if (AdminAccueilFrame.isTableClient()) {
			...
			initColDataCompte();
		else if (ClientAccueilFrame.isTableLog())
			initColDataLog();
		else if (AdminAccueilFrame.isTableAgence())
			initColDataAgence();
	...
	private void initActions() {
		...
		searchPanel.getCrudPanel().deleteBtn().addActionListener(e -> {
			...
			if (row == -1) {
				...
				idClient = (long) TablePanel.getTableModel().getValueAt(row, 0);
